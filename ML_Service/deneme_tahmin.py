...
# TYT denemeleri için dönem bilgileri (örnek verilerdeki ID'lere göre)
DONEMLER = [
    {'id': 1, 'tarih': datetime(2024, 9, 12), 'donem_adi': 'Eylül 2024 (D. 1)'},
    {'id': 7, 'tarih': datetime(2024, 10, 31), 'donem_adi': 'Ekim-Kasım (D. 7)'},
    {'id': 15, 'tarih': datetime(2025, 2, 7), 'donem_adi': 'Şubat 2025 (D. 15)'},
    {'id': 18, 'tarih': datetime(2025, 3, 6), 'donem_adi': 'Mart 2025 (D. 18)'},
    {'id': 21, 'tarih': datetime(2025, 4, 17), 'donem_adi': 'Nisan 2025 (D. 21)'},
]

# TYT 2025 tahmini tarih
...
def banner():
    print(f"""
{Renk.BOLD}{Renk.CYAN}TYT TAHMİN SİSTEMİ v2.1{Renk.RESET}
...
def veri_yukle():
    ...
    try:
        ...
        # Olası tek sütun ayrışım sorununa karşı önlem
        ...
        # Gerekirse toplam sütunlarını hesapla
        donemler_id = [1, 7, 15, 18, 21]
        for donem in donemler_id:
            toplam_col = f'toplam_{donem}'
            turkce = f'turkce_{donem}'
            matematik = f'matematik_{donem}'
            fen = f'fen_{donem}'
            sosyal = f'sosyal_{donem}'
            
            # Eğer toplam sütunu yoksa oluştur, ama sadece alt testler varsa
            if turkce in df.columns and matematik in df.columns and fen in df.columns and sosyal in df.columns:
                if toplam_col not in df.columns:
                     df[toplam_col] = df[turkce] + df[matematik] + df[fen] + df[sosyal]
        
        df = df.replace([np.inf, -np.inf], np.nan)
        # Sadece TÜM deneme verileri boş olan öğrencileri sil, bir dersi dahi boş olsa satırı silme
...
def ml_tahmin(df, kullanici_denemeler):
    ...
    for donem_id in sorted(kullanici_denemeler.keys()):
        if donem_id in deneme_sutunlari:
            sutunlar = deneme_sutunlari[donem_id][:4]
...
def sonuclari_goster(denemeler, trend_tahmin, ml_tahmin_sonuc, benzerler, sinav_tarihi):
    ...
    print(f"  {'Dönem':<18} {'Tür':>6} {'Mat':>6} {'Fen':>6} {'Sos':>6} {'TOP':>7}")
    print(f"  {'-'*55}")
    ...
    for donem_id, netler in sirali:
        ...
        for d in DONEMLER:
            if d['id'] == donem_id:
                donem_adi = d['donem_adi'].split('(')[0].strip() # 'Eylül 2024 (D. 1)' -> 'Eylül 2024'
                break
        print(f"  {donem_adi:<18} {netler['turkce']:>6.1f} {netler['matematik']:>6.1f} {netler['fen']:>6.1f} {netler['sosyal']:>6.1f} {Renk.BOLD}{toplam:>7.1f}{Renk.RESET}")
    ...
    print(f"  {'Yöntem':<18} {'Tür':>6} {'Mat':>6} {'Fen':>6} {'Sos':>6} {'TOP':>7}")
    print(f"  {'-'*55}")
    
    if trend_tahmin:
        t = trend_tahmin
        print(f"  {'Trend':<18} {t['turkce']:>6.1f} {t['matematik']:>6.1f} {t['fen']:>6.1f} {t['sosyal']:>6.1f} {t['toplam']:>7.1f}")
    
    if ml_tahmin_sonuc:
        m = ml_tahmin_sonuc
        print(f"  {'ML Modeli':<18} {m['turkce']:>6.1f} {m['matematik']:>6.1f} {m['fen']:>6.1f} {m['sosyal']:>6.1f} {m['toplam']:>7.1f}")
    ...
    if ort:
        print(f"  {'-'*55}")
        print(f"  {Renk.BOLD}{Renk.YESIL}{'ORTALAMA':<18} {ort['turkce']:>6.1f} {ort['matematik']:>6.1f} {ort['fen']:>6.1f} {ort['sosyal']:>6.1f} {ort['toplam']:>7.1f}{Renk.RESET}")
    ...
    for donem_id, netler in sirali:
        ...
        for d in DONEMLER:
            if d['id'] == donem_id:
                donem_adi = d['donem_adi'].split('(')[0].strip()[:10]
                break
...
def hakkinda():
  ...
  TYT Tahmin Sistemi v2.1
  Deneme sonuçlarını analiz ederek TYT net tahmini yapar.

  {Renk.BOLD}Dönemler:{Renk.RESET}
  - Eylül: Dönem başı
  - Ekim-Kasım: 1. dönem ortası
  - Şubat: 2. dönem başı
  - Mart: Son süreç
  - Nisan: Sınav öncesi
